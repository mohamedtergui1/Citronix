...
public class HarvestDetailsServiceImpl implements HarvestDetailsService {
    ...
    public final TreeMapper treeMapper;

    @Override
    public List<TreeResponseNew> getHarvestDetailsByTreeId(Long id) {
        if (!treeRepository.existsById(id)) {
            throw new NotFoundException("tree not found");
        }
        return treeRepository.findByFieldFarmId(id).stream().map(treeMapper::toTreeResponseNew).toList();
    }
